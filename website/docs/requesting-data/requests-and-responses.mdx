...
o cliente do SDK cria esses objetos para você "magicamente", e vamos explicar como isso funciona nas seções abaixo.
...
// magicamente numa mesma linha: `posts` é um ModelInstance
// e `articles` é um ModelCategoryInstance.
//
// As quatro linhas abaixo são equivalentes
...
const response = await Starlight.model('posts').categories.articles.entries()
const response = await Starlight.posts.categories.articles.entries() // Mágica dupla!
```

Note que você só pode usar essa sintaxe diretamente no cliente para criar ModelInstances. Para criar outros tipos de
Instance, você usa essa sintaxe em Selectors que dão suporte à ela. Nas próximas páginas, você vai entender quais
Selectors suportam essa sintaxe, e qual Instance é retornada quando você a usa.

Em alguns casos, por exemplo, você pode ter que requisitar dados de um *modelo* do qual você não vai saber o slug de
...
// ❌ Não recomendamos usar a sintaxe "mágica":
const response = await Starlight.model[model].entries.get(entry)
```

Apesar da sintaxe "mágica" funcionar, ela pode parecer esquisita para iniciantes, por parecer que estamos acessando um
...
- [StarlightItemResponse](#starlightitemresponse)
- [StarlightListResponse](#starlightlistresponse)
- StarlightError
...
Um objeto StarlightError pode ser definido usando TypeScript dessa maneira:
...
O parâmetro `response` desse objeto é o [objeto Response retornado pela chamada fetch](https://developer.mozilla.org/en-US/docs/Web/API/Response)
